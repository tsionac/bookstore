...
public abstract class MicroService implements Runnable {
    ...
    private ConcurrentHashMap<Class<? extends Message>, Callback> callForMessage;
    private MessageBusImpl msbi;
    ...
    public MicroService(String name) {
        this.name = name;
        this.callForMessage = new ConcurrentHashMap<>();
        this.msbi = MessageBusImpl.getInstance();
    }
    ...
    protected final <T, E extends Event<T>> void subscribeEvent(Class<E> type, Callback<E> callback) {
        msbi.subscribeEvent(type, this);
        callForMessage.put(type, callback);
    }
    ...
    protected final <B extends Broadcast> void subscribeBroadcast(Class<B> type, Callback<B> callback) {
        msbi.subscribeBroadcast(type, this);
        callForMessage.put(type, callback);
    }
    ...
    public final void run() {
        ...
        while (!terminated) {
            try {
                Message message= msbi.awaitMessage(this);
                callForMessage.get(message.getClass()).call(message);
            } catch (InterruptedException e) {
                e.printStackTrace();
            }
        }

        msbi.unregister(this);
